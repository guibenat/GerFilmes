...
public class AutenticacaoService {
    ...
    public AuthResponse cadastrar(CadastroRequest request) {
        ...
        // Cargo.USUARIO: todo cadastro publico e sempre USUARIO
        Usuario usuario = new Usuario();
        usuario.setNome(request.nome());
        usuario.setEmail(request.email());
        usuario.setSenha(passwordEncoder.encode(request.senha())); // hash BCrypt
        usuario.setCargo(Cargo.USUARIO);
        usuarioRepository.save(usuario);

        // Retorna token JWT ja apos o cadastro (usuario ja fica logado)
